class Tracklist < ApplicationRecord
  ...
  def artist_attributes=(artist_attributes)
    artist = Artist.find_by(name: artist_attributes[:name]) || Artist.create(name: artist_attributes[:name], img_url: artist_attributes[:img_url])
    self.artist = artist
  end
  ...
  def order_by_tracklist_number
    songs.sort_by { |song| song.tracklist_number }
  end

  def first_three_songs
    order_by_tracklist_number.first(3)
  end
